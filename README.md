# 🌐 Terrestrial's Personal Portfolio

**🚀 Live Site:** [terrestrial-portfolio}(https://terrestrialyt.com/)

Welcome to my portfolio repository! Everything you see here was designed and built from scratch.

### 🤝 Want Your Own Portfolio?
If you like this design and want me to build a custom portfolio for you, **reach out and let's build it!**

---

### 👨‍💻 About Me
I'm **Terrestrial**, a developer who bridges the gap between software and hardware. 
* 🔭 Founder of **Tytgear.in** (custom desk setups & gear).
* 🛠️ Builder of DIY IoT projects and custom mechanical keyboards.
* 💻 Skilled in **Java, Python, ESP32**, and **Digital Marketing**.

### 📫 Let's Connect
Want to collaborate, talk tech, or hire me to build your portfolio? 
Reach out via my socials: [Linktree](https://linktr.ee/terrestrialyt)
or u can go to my portfolio and get all my social there as well 
